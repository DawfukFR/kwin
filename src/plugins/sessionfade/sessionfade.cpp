...
void SessionFadeEffect::saveCurrentState()
{
    ...
    for (auto screen : workspace()->outputs()) {
        ...
        // if (auto texture = GLTexture::upload(QImage("/home/apol/green.png"))) {
...
void SessionFadeEffect::paintWindow(const RenderTarget &renderTarget, const RenderViewport &viewport, EffectWindow *w, int mask, QRegion region, WindowPaintData &data)
{
    if (m_timeLine.direction() == TimeLine::Forward) {
        // data.setOpacity(m_timeLine.value());
    }
