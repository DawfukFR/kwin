...
bool EglGbmLayerSurface::endRendering(const QRegion &damagedRegion)
{
    if (m_surface->colormanagementEnabled) {
        ...
        m_surface->shadowSwapchain->release(m_surface->currentShadowSlot, std::move(fence.fileDescriptor()));
    ...
    const auto buffer = importBuffer(m_surface.get(), m_surface->currentSlot.get(), std::move(sourceFence.fileDescriptor()));
...
std::shared_ptr<DrmFramebuffer> EglGbmLayerSurface::importWithEgl(Surface *surface, GraphicsBuffer *sourceBuffer, FileDescriptor &&readFence) const
{
    ...
    m_eglBackend->makeCurrent();
    return m_gpu->importBuffer(slot->buffer(), std::move(endFence.fileDescriptor()));
}
