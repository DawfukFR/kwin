...
protected:
    ...
    void kde_output_configuration_v2_set_edid_color_profile(Resource *resource, wl_resource *outputdevice, uint32_t enable) override;
};
...
void OutputConfigurationV2Interface::kde_output_configuration_v2_set_edid_color_profile(Resource *resource, wl_resource *outputdevice, uint32_t enable)
{
    if (invalid) {
        return;
    }
    if (OutputDeviceV2Interface *output = OutputDeviceV2Interface::get(outputdevice)) {
        config.changeSet(output->handle())->edidColorProfile = enable;
    }
}
