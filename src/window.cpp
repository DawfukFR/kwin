    ...
    const qreal titlebarHeight, const Gravity gravity = Gravity::None, int requiredPixels = 100)
{
    ...
    const auto visibleTopLeftPositionToAnchorPosition = [gravity, titlebarHeight, requiredPixels, windowGeom, availableArea](QRect rect) {
        // If rect becomes invalid at any stage of processing, we should early return QRect().
    ...
    for (QRect rect : availableRegion) {
        rect = visibleTopLeftPositionToAnchorPosition(rect);
        if (!rect.isValid()) {
            continue;
        }
